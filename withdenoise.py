...
def insertion_sort(new_list):
    ...
    for _i in range(1, size):
        _j = 1
        while _j > 0 and new_list[_j - 1] > new_list[_j]:
            new_list[_j - 1], new_list[_j] = new_list[_j], new_list[_j - 1]
            _j -= 1
...
def locate_neighbors(new_pixels, row, column, width, height, reach):
    ...
    for row_val in range(2*int(reach) + 1):
        for col_val in range(2*int(reach) + 1):
            ...
            if row_final >= height or row_final < 0:
                continue
            #_row, _col = (row_final), (col_final)
            final_list = new_pixels[row_final][col_final]
            neighbors.append(final_list)
    return neighbors


def denoise(grouped, width, height, reach, beta, header):
    ...
    for row in range(height):
        for column in range(width):
            neighbors = locate_neighbors(new_pixels, row, column, 
            width, height, reach)
            ...
            new_pixels[0] = int(med)
            new_pixels[1] = int(new_pixels[0])
            new_pixels[2] = int(new_pixels[0])
    return new_pixels
